import { DEFAULT_BASE_URL, DEFAULT_TIMEOUT_MS } from '../config/constants.js';
  ...
  async get<T>(path: string): Promise<ApiResult<T>> {
    ...
    if (response.status === 401) {
      return {
        ...
        kind: 'unauthorized',
        message:
          'The API key does not have access to this endpoint (read-only keys cannot access this data, or the key is invalid).'
      };
    }
