...
import { detectKeyTier, type KeyTier } from './tier.js';
import { registerTools } from './tools.js';
import { createServer, createClient } from './server-factory.js';
...
async function main() {
    ...
    try {
        ...
        console.error(error instanceof Error ? error.message : 'Failed to detect Healthchecks.io API key tier.');
        process.exit(1);
        return;
    }
    if (tier === 'read-only') {
