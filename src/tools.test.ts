...
test('list_checks returns the checks array, including an empty account', async () => {
    const client = await connectedClient('read-write', (async () => jsonResponse({ checks: [] })) as typeof fetch);
    try {
...
test('get_check on an invalid id returns a clean tool error, not a crash', async () => {
    const client = await connectedClient(
        'read-write',
        (async () => new Response('<html>Not Found</html>', { status: 404, headers: { 'content-type': 'text/html' } })) as typeof fetch
    );
    try {
...
test('list_check_pings degrades cleanly on a read-only key, without calling the API', async () => {
    ...
    try {
        const result = await client.callTool({ name: 'list_check_pings', arguments: { check_id: 'abc' } });
        assert.equal(result.isError, true);
        assert.match((result.content as Array<{ text: string }>)[0].text, /read-write Healthchecks\.io API key/);
...
test('list_check_pings resolves a unique_key to a uuid before calling /pings/ (real HC.io behavior)', async () => {
    ...
    try {
        const result = await client.callTool({ name: 'list_check_pings', arguments: { check_id: 'the-unique-key' } });
        assert.equal(result.isError, undefined);
        assert.deepEqual(JSON.parse((result.content as Array<{ text: string }>)[0].text), [{ type: 'success' }]);
